...
- `mapillary` — street-level imagery (`streets in Chicago`, `driving through Sweden`, `streets around the world`). Coverage-aware z=14 vector-tile sampling via `MapillaryImporter`: it fetches *every* low-zoom `sequence` tile that tiles the region (at the highest zoom whose full tiling fits a cap — a city ≈ z11, a country ≈ z6; continents/world stratified-sample) to map where imagery actually exists, then fetches z=14 `image` tiles only at covered locations within the region polygon, spread across the region and spatially thinned (so a country spreads nationwide and a park spreads along roads instead of clustering). Requires `MAPILLARY_TOKEN` in env. Panoramas are excluded.

**Sub-region modes** (consumed by `RegionResolver`):

- *Mode A* — in-DB named region (`world` / continent / country / admin1 / admin2 / city). `world` ("streets around the world") returns the global bbox; the rest trigger a Nominatim boundary fetch (point-bbox seeds **and** countries/states, so Commons/Mapillary refine against the real polygon instead of leaking into neighbours sharing the bbox).
