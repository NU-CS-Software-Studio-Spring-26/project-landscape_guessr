...
- `commons` — high-volume / single-subject prompts (`Mount Fuji photos`, `many photos of buildings in Boston`). Topic Q-ID → P373 → Commons category, then CirrusSearch `deepcategory:` + `nearcoord:` via `CommonsImporter`; when no Q-ID has a clean P373 the AI supplies a single distinctive title keyword (`skyline`, not the phrase `city skylines`) used as `intitle:`. For topic+region it builds the region-anchored category (`Buildings in Boston`); region-less subjects are anchored on the topic's own P625 coordinate. The region is covered by a hex lattice of `nearcoord:` probes whose **radius scales with region size** so a country tiles with ~200 large covering circles instead of stranding every probe in one corner; a probe is kept when its circle (not just its centre) overlaps the region polygon, so coastal cities aren't dropped, and `collect_rows` caps each probe at its fair share of the budget so a photo-dense city can't dominate the set. Only geotagged files are kept (the `coordinates` prop is paged with `colimit` so it isn't capped at 10 per request), non-photographic files (relief maps, satellite tiles, schematics) are dropped via the shared `MediaFilter` guard that Wikidata also uses, and display URLs are `Special:FilePath/<file>` (thumb size applied by `image_src ?width`) — not server-rendered thumbnails, which would break pagination.
...
Directional sub-region splits (`north half of Chicago`), multi-region prompts, and named routes are refused with a redirect to the existing filter-by-area workflow. A country whose territory crosses the 180° meridian or scatters across oceans (the US via the Aleutians + Hawaii + American Samoa, Russia via Chukotka, France via Kerguelen + New Caledonia) otherwise gets a degenerate ~360°-wide bbox that corrupts the tile/probe math. `RegionResolver#primary_landmass_bbox` repairs it: it keeps the **dominant landmass cluster** — the largest constituent polygon plus parts within ~1500km of it whose union still spans ≤180° — and recomputes the search bbox from that, so `US city skylines` / `streets in Russia` / `streets in New Zealand` work instead of being refused. The full polygon is still used for filtering; only the far-flung minority-hemisphere slivers (the +180° Aleutian islet, the Chatham Is., French Polynesia) go unsearched. The pipeline's >180° guard remains as a last-resort fallback for the rare region with no usable polygon.
